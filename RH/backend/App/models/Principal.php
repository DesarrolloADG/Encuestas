...
class Principal implements Crud{
  ...
  INNER JOIN cuestionario c ON ca.id_cuestionario = c.id_cuestionario WHERE cc.id_colaborador = $id AND cc.estatus = 0
...
sql;
    ...
    public static function getObtenerColaborador($id){

        $mysqli = Database::getInstance();
        $query=<<<sql
        SELECT id_colaborador FROM usuarios_encuestas
        WHERE usuario = '$id'
